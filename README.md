...
- **📔 Journals (travel diary, >= 0.25.0):** Keep a hand-written travel diary in the new **Journeys** sidebar app: create a Journal, add a day entry (auto-filled with up to 20 photos spread evenly across that day), write text, curate/reorder the photos, and caption each one. Publish a Journal to a revocable public link — a mobile-friendly timeline with per-day locations, a travel map (an OpenStreetMap basemap with the route between geolocated days, rendered and cached server-side), a visited countries/cities overview, a cover photo, a newest-day-first timeline with an oldest/newest sort toggle, and a prev/next photo lightbox. Share a Journal with other instance users or groups for full co-editing; each contributor adds photos from their own library.
